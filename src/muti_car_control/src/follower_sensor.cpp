...
void IMUCallback(sensor_msgs::Imu msg)
{
    ...
    double pitchAngVelocity = msg.angular_velocity.y;
    //double yawAngVelocity = yaw - lastYaw;
    double yawAngVelocity = msg.angular_velocity.z;
    
    // Load request message
    balance_car_control::control_param cp;
...
int main(int argc, char **argv) {
    ...
    client = nh.serviceClient<balance_car_control::control_param>("control_param");
    ros::service::waitForService("control_param");

    // Control the wheel output torque
    wheelsCommandPub = nh.advertise<std_msgs::Float64MultiArray>("/follower/wheels_controller/command", 1);

    ros::Subscriber imu_sub = nh.subscribe("/follower/imu/data", 10, IMUCallback);

    // Subscription of speed information for two wheels for feedback control
    ros::Subscriber wheelVelocity_sub = nh.subscribe<sensor_msgs::JointState>("/follower/joint_states", 10, WheelVelocityCallback);

    ros::Subscriber keyboard_cmd_sub = nh.subscribe<geometry_msgs::Twist>("/follower/cmd_vel", 10, KeyboardCmd);
