...
int main(int argc, char **argv) {
    ...
    wheelsCommandPub = nh.advertise<std_msgs::Float64MultiArray>("/leader/wheels_controller/command", 1);

    ros::Subscriber imu_sub = nh.subscribe("/leader/imu/data", 10, IMUCallback);

    // Subscription of speed information for two wheels for feedback control
    ros::Subscriber wheelVelocity_sub = nh.subscribe<sensor_msgs::JointState>("/leader/joint_states", 10, WheelVelocityCallback);

    ros::Subscriber keyboard_cmd_sub = nh.subscribe<geometry_msgs::Twist>("/leader/cmd_vel", 10, KeyboardCmd);
